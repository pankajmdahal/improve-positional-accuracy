...
cwd = os.getcwd()
adf_folder_path = '../../RAIL/netdata/ADF/'
os.chdir(adf_folder_path)
name_list = os.listdir(".")
folder_in_adf = sorted(name_list, key=os.path.getmtime)[-1]
os.chdir(cwd)

#adf with the changed network
adf1 = os.listdir(adf_folder_path + folder_in_adf)
...
for i in range(len(adf1)):
    df = create_new_adf_from_adf(adf_folder_path + folder_in_adf+ "/" + adf1[i])
    #finding output foldername
    cwd = os.getcwd()
    adf_folder_path2 = '../../RAIL11/RAIL/netdata/ADF/'
    os.chdir(adf_folder_path2)
    name_list = os.listdir(".")
    folder_in_adf = sorted(name_list, key=os.path.getmtime)[-1]
    os.chdir(cwd)
    df.to_csv(adf_folder_path2 + folder_in_adf + "/" + "imported_from_RAIL_" + str(i) + ".ADF")
